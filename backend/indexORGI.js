...
app.get('/api/superadmin/tasks', authenticate, async (req, res) => {
  try {
    ...
    const [tasks] = await pool.query(
      'SELECT id, user_id AS userId, title, status, time_spent AS timeSpent, deadline, notes, urgent FROM tasks'
    );
    res.json(tasks.map(task => ({
      ...task,
      userId: [task.userId],  // Wrap in array for frontend consistency (expand if using junction table)
...
// Updated: POST /api/superadmin/tasks (handle multiple userIds)
app.post('/api/superadmin/tasks', authenticate, async (req, res) => {
  try {
    ...
    // Insert a task for each userId (simplest approach; adjust for junction table if preferred)
    const taskInserts = userIds.map(userId =>
      pool.query(
        'INSERT INTO tasks (user_id, title, status, time_spent, deadline, notes, urgent) VALUES (?, ?, "To Do", "0h", ?, ?, ?)',
        [userId, title, deadline, notes, urgent ? 1 : 0]
      )
    );
    await Promise.all(taskInserts);

    // Fetch the newly created tasks (assumes single task per user for now)
    const [newTasks] = await pool.query(
      'SELECT id, user_id AS userId, title, status, time_spent AS timeSpent, deadline, notes, urgent FROM tasks ORDER BY id DESC LIMIT ?',
      [userIds.length]
    );

    res.status(201).json(newTasks.map(task => ({
      id: task.id,
      userId: [task.userId],  // Array for consistency
      title: task.title,
      status: task.status,
